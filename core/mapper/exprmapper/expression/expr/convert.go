...
func ConvertToRef(val interface{}) (*ref.MappingRef, error) {
	...
	log.Infof("Convert to ref type %s value %+v", reflect.TypeOf(val), val)
	switch val.(type) {
	case string:
...
//		return t, nil
//	case string:
//		log.Debug("Convert function from string.")
...
//		return function, nil
//	default:
//		log.Debugf("Convert function from type %s", reflect.TypeOf(val))
